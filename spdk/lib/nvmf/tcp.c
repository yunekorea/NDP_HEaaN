...
//		if (cmd->opc == SPDK_NVME_OPC_CUSTOM_GREP) {
//
//            if (spdk_nvmf_request_get_buffers(req, group, transport, length)) {
//                /* No available buffers. Queue this request up. */
//                SPDK_DEBUGLOG(nvmf_tcp, "No available large data buffers. Queueing request %p\n",
			...
			struct spdk_nvmf_tcp_req *tcp_req)
{
    ...
    for (int i = 0; i < tcp_req->req.iovcnt && tc < data_len; i++) {
        char *data = (char *)tcp_req->req.iov[i].iov_base;
        uint32_t iov_len = tcp_req->req.iov[i].iov_len;
...
//	printf("===Debugging PDU setup====:\n");
//    printf("req->iovcnt: %d\n", tcp_req->req.iovcnt);

//    for (int i = 0; i < tcp_req->req.iovcnt; i++) {
//        printf("iov[%d]: base=%p, len=%lu\n",
//                      i, tcp_req->req.iov[i].iov_base, tcp_req->req.iov[i].iov_len);
//    }

//    printf("c2h_data->datao: %u\n", c2h_data->datao);
//    printf("c2h_data->datal: %u\n", c2h_data->datal);
//    printf("rsp_pdu->rw_offset: %u\n", tcp_req->pdu->rw_offset);
//    printf("plen: %u, pdo: %u, padding_len: %u\n",
//                  plen, c2h_data->common.pdo, rsp_pdu->padding_len);

	nvme_tcp_pdu_set_data_buf(rsp_pdu, tcp_req->req.iov, tcp_req->req.iovcnt,
				  c2h_data->datao, c2h_data->datal);

//	printf("rsp_pdu->data_iov setup before/after nvme_tcp_pdu_set_data_buf call:\n");
//    for (int i = 0; i < rsp_pdu->data_iovcnt; i++) {
//        printf("iov[%d]: base=%p, len=%lu\n",
//                      i, rsp_pdu->data_iov[i].iov_base, rsp_pdu->data_iov[i].iov_len);
//    }
...
nvmf_tcp_send_c2h_data(struct spdk_nvmf_tcp_qpair *tqpair,
		       struct spdk_nvmf_tcp_req *tcp_req)
{
//    fprintf(stdout, "nvmf_tcp_send_c2h_data called\n");
