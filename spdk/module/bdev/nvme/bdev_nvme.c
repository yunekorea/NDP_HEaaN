...
//static int bdev_nvme_custom_echo(struct nvme_bdev_io *nbdev_io,
//                      struct iovec *iovs, int iovcnt,
//                      uint64_t num_blocks, uint64_t offset_blocks);
...
_bdev_nvme_submit_request(struct nvme_bdev_channel *nbdev_ch, struct spdk_bdev_io *bdev_io)
{
	...
	case SPDK_BDEV_IO_TYPE_WRITE:
		...
		break;
//	case SPDK_BDEV_IO_TYPE_CUSTOM_ECHO:
//        rc = bdev_nvme_custom_echo(nbdev_io,
//                                   bdev_io->u.bdev.iovs,
//                                   bdev_io->u.bdev.iovcnt,
//                                   bdev_io->u.bdev.num_blocks,
//                                   bdev_io->u.bdev.offset_blocks);
...
//static int
//bdev_nvme_custom_echo(struct nvme_bdev_io *nbdev_io,
//                      struct iovec *iovs, int iovcnt,
//                      uint64_t num_blocks, uint64_t offset_blocks)
//{
//    struct spdk_nvme_ns *ns = nbdev_io->io_path->nvme_ns->ns;
//    struct spdk_nvme_qpair *qpair = nbdev_io->io_path->qpair;
//
//    return spdk_nvme_ns_cmd_readv(ns, qpair,
//                                  offset_blocks, num_blocks,
//                                  bdev_nvme_io_complete_cb, nbdev_io,
//                                  0,
//                                  bdev_nvme_io_get_buf, bdev_nvme_io_get_next_sge,
//                                  NULL, 0, 0);
//}
